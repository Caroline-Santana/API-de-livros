/*****************************************************
 * Objetivo: Listar as informações dos livros
 * Data: 08/09/2022
 * Autor: Caroline A.
 *  * Versão: 1.0
 *****************************************************/
...
const buscandoWords = function(WordLocker){
    let palavraChave = WordLocker.toLowerCase()
   let listaLivros = []
   let totalLivros = {}
   let erro = true 
        ...
        if(palavraChave != ''){
            livros.forEach(item => {
                item.books.forEach(item => {
                    const filterNames = item.title.toLowerCase().includes(palavraChave)
                    if(filterNames == true){
                           listaLivros.push
                           ({
                                'Título: ':item.title,
                                ...
                                'Preço: ':item.price,
                                'Foto da capa: ':item.image
                            })
                           
                        erro = false
                    }

                    totalLivros.Total = listaLivros.length
                    totalLivros.Livros = listaLivros
                })
            })
        }
    }
